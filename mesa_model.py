...
class Scientist(mesa.Agent):
    ...
    def increase_prestige(self, value):
        ...
        self.prestige_visibility += np.mean(
            [
                ...
                for a in self.model._scientists
                if a != self
            ]
        )
    ...
    def _compute_visibility(self, avg_agent_distance: np.float64) -> np.float64:
        return (self.model.agent_avg_distance + 1.0) / (avg_agent_distance + 1.0)
    ...
    def compute_all_rewards(
        ...
        others = [a for a in self.model._scientists if a is not self]
        dists = np.fromiter(
            (a.compute_distance(pos) for a in others),
            dtype=np.float64,
            count=len(others),
        )
        prest = np.fromiter(
            (a.prestige_vanishing for a in others), dtype=np.float64, count=len(others)
        )

        avg_agent_distance = dists.mean()
        visibility = self._compute_visibility(avg_agent_distance)
        ...
        visibility_used = visibility
        if self.model.use_visibility_reward:
            visibility_used = (self.model.agent_avg_distance + 1) / (
                avg_agent_distance_p + 1
            )

        # differentiate between visibility of a field vs. visibility of an agent?
        # (would not this formulation stay constant across neihbouring cells?; answer: no, it won't)
        reward = (novelty / self.model.avg_current_agent_knowledge) ** (
            self.curiosity
        ) * visibility_used ** (1 - self.curiosity)
...
class MyModel(mesa.Model):
    number_agents: int
    agent_generation_time: np.float64
    new_questions: bool
    size: int
    steps: int
    harvest: np.float64
    ...
    epsilon: np.float64
    use_distance: bool
    _scientists: list[Scientist]
    total_initial_knowledge: np.float64
    avg_current_agent_knowledge: np.float64
    agent_avg_distance: np.float64

    def _init_grid(
        self, cell_init_function: Callable[[int, int, int, Any, Generator]]
    ):
        ...
        self.grid.add_property_layer(
            PropertyLayer(
                "previous_knowledge", self.size, self.size, np.float64(0.0), dtype=np.float64
            )
        )
        self.grid.add_property_layer(
            PropertyLayer(
                "initial_knowledge", self.size, self.size, np.float64(0.0), dtype=np.float64,
            )
        )
        self.grid.add_property_layer(
        ...
        for pos_x, pos_y in list(itertools.product(range(self.size), range(self.size))):
            ...
            self.grid.properties["previous_knowledge"].data[pos_x, pos_y] = (
                self.grid.properties["knowledge"].data[pos_x, pos_y]
            )
            self.grid.properties["explored"].data[pos_x, pos_y] = False
        self.total_initial_knowledge = np.float64(1.0)

    def _init_agents(self, agent_generation_function):
    ...
    def __init__(
        ...
        harvest: np.float64,
        grid_size: int,
        cell_init_function: Callable[[int, int, int, Dict, Generator], np.float64],
        use_distance: bool,
        generation_params={"seed": 0},
        agent_generation_time: np.float64 = np.float64(-1.0),
        new_questions: bool = False,
        agent_seed: int = 0,
        step_limit: int = 400,
        agent_generation_function: Callable[
            [np.float64, Generator, Dict], np.float64
        ] = lambda cur, rng, params: cur,
        vanishing_factor_for_prestige: np.float64 = np.float64(0.0),
        use_visibility_reward: bool = True,
    ):
        ...
        self.generation_params = generation_params | {
            "initCellFunc": cell_init_function.__name__, "seed": self._seed
        }
        self.number_agents = n_agents
        self.agent_generation_time = agent_generation_time
        ...
        self._init_grid(cell_init_function)
        self._init_agents(agent_generation_function)
        ...
        self.datacollector = mesa.DataCollector(
            model_reporters={
                ...
                "avg_knowledge_on_grid": lambda m: np.mean(
                    m.grid.properties["knowledge"].data, dtype=np.float64
                ),
                ...
                "Corr_prestige_local_merit": correlation_reporter("current_local_merit", "prestige"),
                "Corr_prestige_ideal_merit": correlation_reporter("ideal_Merit", "prestige"),
                "Corr_prestige_visibility_local_merit": correlation_reporter("current_local_merit", "prestige_visibility"),
                "Corr_prestige_visibility_ideal_merit": correlation_reporter("ideal_Merit", "prestige_visibility"),
                "Corr_prestige_vanishing_local_merit": correlation_reporter("current_local_merit", "prestige_vanishing"),
                "Corr_prestige_vanishing_ideal_merit": correlation_reporter("ideal_Merit", "prestige_vanishing"),
            },
            agent_reporters={
                "prestige": "prestige",
                ...
                "curiosity": "curiosity",
                "local_merit": "current_local_merit",
            },
        )
    ...
    def update_knowledge(self):
        self.avg_current_agent_knowledge = np.mean(
            [agent.last_tile_knowledge for agent in self._scientists], dtype=np.float64
        )
    ...
    def updateIdealMerit(self):
        """ computes the loss for each agent compared to the best position they could have in the ideal harvest situation"""
    ...
    def __generate_new_agents(self):
        if self.agent_generation_time > 0:
            agents_to_generate: int = np.floor(
                self.steps * self.agent_generation_time
            ) - np.floor((self.steps - 1) * self.agent_generation_time)
            prestige_agent_tuples = [(a.prestige, a) for a in self._scientists]
            totalPrestige = sum([a.prestige for a in self._scientists])
            supervisors = []
            ...
            for sup in supervisors:
                ...
                coords = sup.pos
                self.grid.place_agent(a, coords)
                self._scientists.append(a)
            for agent in self._scientists:
    ...
    def _generate_new_questions(self):
        if self.new_questions:
            ...
            for pos_x, pos_y in occupied_pos:
                if self.grid.properties["knowledge"].data[pos_x, pos_y] < 0.025:
                    self.grid.properties["knowledge"].data[pos_x, pos_y] = (
                        self.rng.random()
                    )

    def _update_step_goals(self) -> None:
    ...
    def end_loop_update(self):
        ...
        self.agent_avg_distance = np.mean(
            [
                np.mean(
                    [a.compute_distance(b.pos) for b in self._scientists if a != b],
                    dtype=np.float64,
                )
                for a in self._scientists
            ],
            dtype=np.float64,
        )
    ...
    def get_report(self):
        a = {
            k: self.datacollector.get_model_vars_dataframe()[k].iloc[-1]
            for k in self.datacollector.get_model_vars_dataframe().columns 
            if k[0].islower()
        }
        b = {
        ...
        c = {
            "Mean_of_"+k: self.datacollector.get_model_vars_dataframe()[k].mean()
            for k in self.datacollector.get_model_vars_dataframe().columns 
            if k[0].isupper()
        }
        return {**a, **b, **c}
